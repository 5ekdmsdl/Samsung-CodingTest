def move(n) :
    ...
    moving = stack[idx:]
    mapp[nx][ny] += moving
    if len(mapp[nx][ny]) >= 4 : done = 1
    mapp[x][y] = mapp[x][y][:idx]
    if len(mapp[x][y]) >= 4 : done = 1
...
def blue(n):
    ...
    x, y, d = players[n]
    nx, ny = x + dx[d], y + dy[d]
    if not (0 <= nx <= nn-1 and 0 <= ny <= nn-1) : return
...
while(t < 1000) :
    ...
    for i, p in enumerate(players) :
        ...
        if not (0 <= nx <= nn-1 and 0 <= ny <= nn-1) :
            ...
            if cmapp[nx][ny] == 0 : white(i)
            elif cmapp[nx][ny] == 1: red(i)
            elif cmapp[nx][ny] == 2 : blue(i)
        if done :
            ddone = 1
            break
    if ddone : break
